package dao_dataAccessObject.implementation;

import dao_dataAccessObject.AbstractDao;
...
// here must be done clearing catches after this operation!!!
    @Override
    ...
    @Override
    public HashSet<AppUser> getFollowedUser(String login) {
        AppUser userByLogin = getUserByLogin(login);
        return new HashSet<>(userByLogin.getFollowedByUser());
    }

    @Override
    public HashSet<AppUser> getNotFollowedUser(String login) {
        ...
        users.removeAll(getFollowedUser(login));
